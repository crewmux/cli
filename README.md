...
- macOS 권장
- Linux도 CLI와 `cm web`은 사용 가능하지만 서비스 설치 헬퍼는 아직 없음
...
`./install.sh`는 위 의존성을 자동으로 맞추려 시도합니다. 기본값은 `claude` + `codex` 둘 다 설치이며, 특정 provider만 원하면 `CM_INSTALL_AGENTS=claude ./install.sh` 또는 `CM_INSTALL_AGENTS=codex ./install.sh`처럼 제한할 수 있습니다. 기존 `AI_INSTALL_AGENTS`도 계속 인식합니다.
...
비대화식 설치(`curl | bash`)에서는 서비스 설치도 기본으로 함께 진행됩니다. 원하지 않으면 `CM_INSTALL_SERVICE=0 ./install.sh`를 사용하세요. 기존 `AI_INSTALL_SERVICE`도 계속 인식합니다.
...
5. `~/.local/bin/cm` 설치
6. `~/.local/bin/ai` 호환용 링크 생성
7. `~/.crewmux/` 데이터 디렉토리 준비
8. 선택적으로 `cm install` 실행
...
ln -sf ~/.local/bin/cm ~/.local/bin/ai
```
...
cm task spawn -t codex -m gpt-5.3-codex "Fix the login bug"
cm task spawn -t claude -n 2 "Write regression tests"
```
...
cm ctl send master "Summarize worker progress"
...
- 세션 이름은 프로젝트 디렉토리명에서 자동 생성됩니다. 예: `/Users/ko/my-project` -> `cm-my-project`
- `cm task *`와 `cm ctl *` 명령은 현재 작업 디렉토리 기준으로 세션을 찾습니다
- 따라서 세션을 시작한 프로젝트 디렉토리 안에서 제어 명령을 실행하는 것이 기본 전제입니다
- `cm team start -t <provider> -m <model>`로 master provider/model을 지정할 수 있습니다
- CLI의 `cm task spawn`은 태스크 문구가 필수입니다
- 웹 UI/API는 태스크 없이 idle worker를 띄우는 것도 허용합니다
- `cm ctl interrupt all`과 `POST /api/interrupt`의 `target=all`은 master + 모든 worker에 Ctrl+C를 보냅니다. log pane은 제외됩니다
- 기존 `ai-*` tmux 세션과 `~/.ai-team` 메타데이터도 자동으로 읽어 하위 호환합니다
...
- `cm team stop-all`은 이름이 `cm-` 또는 legacy `ai-`로 시작하는 tmux 세션을 모두 종료합니다
- Codex는 프로젝트별 trust 설정이 필요한데, `cm`이 스폰 시 해당 프로젝트를 자동으로 trusted로 등록합니다
