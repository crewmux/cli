...
이 문서는 `ai`를 처음 설치하고 실제 프로젝트에서 첫 팀 세션을 띄우는 최소 절차를 정리합니다. 문서 예시는 macOS 기준이며, Linux에서는 서비스 설치 부분만 수동으로 대체하면 됩니다.
...
중요: 이제 `ai team start`에서 master provider를 고를 수 있습니다. `claude`만 있으면 Claude master, `codex`만 있으면 Codex master로 시작하면 됩니다.
...
AI_INSTALL_SERVICE=0 ./install.sh
```
...
- log pane (`tail -f ~/.ai-team/logs/<session>.log`)
- `~/.ai-team/tasks/<session>/meta.json`
- `~/.ai-team/master-prompt.md` 기본 템플릿(없을 때만)

예전 기본 prompt가 남아 있으면 `.legacy.bak`로 백업 후 새 템플릿으로 교체됩니다.

세션 이름은 현재 디렉토리명에서 계산됩니다. 예를 들어 `/Users/ko/work/api-server`에서 시작하면 세션 이름은 `ai-api-server` 입니다.
...
ai task spawn -t claude -n 2 "Write tests for src/auth.rs"
```

CLI에서는 `ai task spawn`의 태스크 문자열이 필수입니다.
...
```bash
ai task master "Summarize current progress"
ai task send codex-1 "Focus on the edge case handling"
ai ctl broadcast "Stop and report current status"
```
...
- `ai task *`, `ai ctl *`는 현재 디렉토리 기준으로 세션을 찾습니다
- 다른 디렉토리에서 `ai ctl status`를 치면 `No active team session`이 나올 수 있습니다
- 이 경우 세션을 시작했던 프로젝트 폴더로 다시 `cd`해서 실행해야 합니다
- 웹 UI는 idle worker 생성이 가능하지만 CLI는 빈 태스크를 허용하지 않습니다
- master는 기본 conflict-avoidance prompt를 자동으로 사용합니다. 필요하면 `~/.ai-team/master-prompt.md`를 수정하세요
