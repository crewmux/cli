...
이 문서는 `CrewMux`를 처음 설치하고 실제 프로젝트에서 첫 팀 세션을 띄우는 최소 절차를 정리합니다. 기본 명령어는 `cm`이며, 문서 예시는 macOS 기준입니다. Linux에서는 서비스 설치 부분만 수동으로 대체하면 됩니다.
...
중요: 이제 `cm team start`에서 master provider를 고를 수 있습니다. `claude`만 있으면 Claude master, `codex`만 있으면 Codex master로 시작하면 됩니다.
...
CM_INSTALL_SERVICE=0 ./install.sh
```
...
위 출력이 정상이고 `tmux`, `claude`가 shell에서 직접 실행되면 첫 실행 준비가 끝난 상태입니다. 기존 `ai --help`도 호환용으로 동작합니다.
...
세션 이름은 현재 디렉토리명에서 계산됩니다. 예를 들어 `/Users/ko/work/api-server`에서 시작하면 세션 이름은 `cm-api-server` 입니다.
...
cm task spawn -t claude -n 2 "Write tests for src/auth.rs"
```

CLI에서는 `cm task spawn`의 태스크 문자열이 필수입니다.
...
```bash
cm task master "Summarize current progress"
cm task send codex-1 "Focus on the edge case handling"
cm ctl broadcast "Stop and report current status"
```
...
- `cm task *`, `cm ctl *`는 현재 디렉토리 기준으로 세션을 찾습니다
- 다른 디렉토리에서 `cm ctl status`를 치면 `No active team session`이 나올 수 있습니다
