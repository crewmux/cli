...
AI CLI를 여러 개 동시에 돌리려면 보통 여러 터미널 창과 수동 복사/붙여넣기, 상태 확인, 로그 추적이 필요합니다. `ai`의 목적은 이 과정을 "프로젝트 단위 팀 세션"으로 정리해, 여러 에이전트를 한 번에 띄우고 제어하는 운영 오버헤드를 줄이는 것입니다.
...
- `ai-` prefix 기반 `stop-all`은 보수적으로 관리해야 함
